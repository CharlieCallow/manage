"""Valuation analyst. CLAUDE.md §5, §13."""
...
class ValuationAnalyst(LLMAgent):
    def __init__(self, model: str, prompt_template: str) -> None:
        super().__init__(name="valuation", model=model, prompt_template=prompt_template)

    def build_user_message(self, ctx: AgentContext) -> str:
        return analyst_user_message(
            kind="valuation", ticker=ctx.ticker or "(unspecified)", snapshot=ctx.snapshot
        )
